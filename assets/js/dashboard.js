...
async function loadUserGroups() {
  ...
  try {
    ...
    membershipSnap.forEach(docSnap => {
      ...
      if (data.groupId) groupIds.push(data.groupId);
    });
...
function renderGroupCard(group) {
  const progress = typeof group.progress === 'number' ? group.progress : 0;
  return `
    <div class="group-card" data-group-id="${escapeHtml(group.id)}">
      <div class="group-header">
        ...
        <div class="group-progress">
          <div class="group-progress-label">Progress</div>
          <div class="group-progress-value">${progress}%</div>
        </div>
      </div>
